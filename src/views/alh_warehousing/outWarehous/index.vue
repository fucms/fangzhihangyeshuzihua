    ...
    <div class="filter-container">
      ...
      <el-form :inline="true" :model="listQuery" class="demo-form-inline">
        <el-form-item label="货物名称">
          <el-input v-model="listQuery.filter" placeholder="请输入货物名称" />
        </el-form-item>
        <el-form-item>
      ...
      >
        ...
        <el-table-column label="货物名称" prop="code" align="center">
          <template slot-scope="{ row }">
            <span>{{ row.code }}</span>
          </template>
        </el-table-column>
        ...
        <el-table-column label="规格" prop="type1" align="center">
          <template slot-scope="{ row }">
            <span>{{ row.code }}</span>
          </template>
        </el-table-column>
        <el-table-column label="出库时间" prop="type1" align="center">
          <template slot-scope="{ row }">
            <span>{{ row.code }}</span>
          </template>
        </el-table-column>
        <el-table-column label="出库原因" prop="address" align="center">
          <template slot-scope="{ row }">
            <span>{{ row.code }}</span>
          </template>
        </el-table-column>
